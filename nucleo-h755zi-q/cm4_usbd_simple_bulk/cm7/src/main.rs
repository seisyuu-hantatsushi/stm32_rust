...
fn main() -> ! {
    ...
    // 48MHz CLOCK is inputed into USB CLOCK
    let _ = ccdr.clocks.hsi48_ck().expect("HSI48 must run");

    // Hardware semaphore was reseted by cm4.
    ...
    // finished initializing Peripherals
    info!("cm7 wakes up cm4.");
    sem0.fast_take();
    sem0.release(0);
    loop {
